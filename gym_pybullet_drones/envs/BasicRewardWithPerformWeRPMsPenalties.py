...
class BasicRewardWithPerformWeRPMsPenalties(BaseRLAviary):
    ...
    @staticmethod
    def _mapped_rpms_between_zero_two(rpms):
        return np.array(list(map(lambda rpm: (rpm - 13750) / 750, rpms)))
    ...
    def _computeReward(self):
        ...
        mapped_rpms = self._mapped_rpms_between_zero_two(state)

        # print(f"#### RPMs BEFORE {state[16:20]} RPMs AFTER: {self._mapped_rpms_between_zero_two(state[16:20])} ####")
        ...
        ret = ((25 - 20 * self._target_error(state) -
               ...
               0.5 * (np.linalg.norm(self._mapped_rpms_between_zero_two(state))))
        return ret
